 ...
 * <li>sql/<code>type</code> -- Contains statements specific to a given database type such as
 * "oracle".</li>
 * </ul>
 * 
 * <p>
...
public final class StatementLoader {
	 ...
	 */
	private static Map<LoaderKey, StatementLoader> loaders = new HashMap<LoaderKey, StatementLoader>();
	 ...
	 * @param dialect
	 *          The {@link DIALECT} dialect of the database for the statements.
	 */
	private StatementLoader(final Class owner, final String dialect) {
		this.owner = owner;
		this.dialect = dialect;
		this.supportsInlineComments = StringUtils.equalsIgnoreCase(dialect, ORACLE_DIALECT);
	}
	 ...
	 * @param dialect
	 *          The {@link DIALECT} of the database for the statements.
	 * 
	 * @return A <code>StatementLoader</code> for the specified owner/dialect.
	 */
	public static StatementLoader getLoader(final Class owner, final String dialect) {
	 ...
	 * 
	 * @throws {@link MissingResourceException} if the resource can not be loaded.
	 */
	...
	private String readStatement(final String name, final Class owner) {
		...
		if (null == stream) {
			...
			logger.info("Loading statement from [{}]", resourceName.toString());
		...
		if (supportsInlineComments) {
			// This comment structure is intended for simple use with existing SQL monitoring utilities.
			buffer.append("/*+ JDBC<");
		...
		try {
			...
			try {
				...
				// Ignore any IOException as we are closing the reader.
			}
		}
	...
	private static class LoaderKey {
		...
		public LoaderKey(final Class owner, final String dialect) {
			this.owner = owner;
			this.dialect = dialect;
		}
